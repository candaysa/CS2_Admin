...
public sealed class SilenceCommand : CommandBase
{
    ...
    public override void Execute(ICommandContext context)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            foreach (var target in targetSnapshots)
            {
                ...
                {
                    Core.Scheduler.NextTick(() => Reply(context, "player_already_silenced", target.Name));
                    continue;
                }
                ...
                {
                    BroadcastNotification(adminName, "silenced_notification", target.Name, durationText, reason);
                ...
                await AdminLogManager.AddLogAsync("silence", adminName, adminSteamId, target.SteamId, target.IpAddress, $"duration={duration};reason={reason}", target.Name);
