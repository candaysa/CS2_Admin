...
public class BuryCommand : CommandBase
{
    ...
    public override async void Execute(ICommandContext context)
    {
        try
        {
            var args = NormalizeArgs(context.Args, CommandsConfig.Bury);

            if (!HasPerm(context, Permissions.Bury))
            {
            ...
            _ = AdminLogManager.AddLogAsync("bury", adminName, context.Sender?.SteamID ?? 0, null, null, $"targets={targets.Count}");
            Core.Logger.LogInformationIfEnabled("[CS2_Admin] {Admin} buried {Count} player(s)", adminName, targets.Count);
        }
        catch (Exception ex)
        {
            Core.Logger.LogErrorIfEnabled(ex, "[CS2_Admin] Bury command failed");
        }
    }
}
